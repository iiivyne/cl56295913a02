...
-- SETTINGS
local AIMLOCK_ENABLED = false       -- toggled by H
local BUTTON_ACTIVE = false         -- toggled by the button
...
local function registerFolder(folder)
    if folder and not table.find(NPC_FOLDERS, folder) then
        table.insert(NPC_FOLDERS, folder)
        print("[Aimlock] Registered:", folder:GetFullName())
    end
end

-- Drop NPCs
task.spawn(function()
    registerFolder(workspace:WaitForChild("Drop"):WaitForChild("NPCs"))
end)

-- Mansion Guards
task.spawn(function()
    local mansion = workspace:WaitForChild("MansionRobbery")
    registerFolder(mansion:FindFirstChild("GuardsFolder"))
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    ...
    if input.KeyCode == TOGGLE_KEY then
        if BUTTON_ACTIVE then
            ...
            lockedPart = AIMLOCK_ENABLED and getNearestNPC() or nil
...
plr:CreateButton("fly ('V' to fly, 'B' to change speeds)", function()
	...
	local fast = true
	local FLYING = false
	local CONTROL -- shared reference
